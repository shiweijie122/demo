var App = function() {
	...
	var f = function() {
		if ($(window).width() <= 767) {
			$("body").on(
					"movestart",
					function(y) {
						if ((y.distX > y.distY && y.distX < -y.distY)
	...
	var d = function() {
		...
		$("li:has(ul)", "#sidebar-content ul").each(
				function() {
		...
		if (/Android|webOS|iPhone|iPad|iPod|BlackBerry/i
			...
			if ($("#sidebar").hasClass("sidebar-fixed")
				...
				if (x && !$("#sidebar").hasClass("sidebar-fixed-responsive")) {
					...
					if ($(window).width() <= 767) {
						$("#sidebar").css("margin-left", "-250px").css(
								"margin-top", "-52px")
					}
				} else {
					var y = 7;
	...
	var k = function() {
		$(".table-checkable thead th.checkbox-column :checkbox").on(
				"change",
				function() {
					var z = $(this).prop("checked");
					var x = $(this).parents("table.table-checkable").data(
							"horizontalWidth");
					if (typeof x != "undefined") {
						var y = $(this).parents(".dataTables_scroll").find(
								".dataTables_scrollBody tbody")
					} else {
						var y = $(this).parents("table").children("tbody")
					}
					y.each(function(B, A) {
						$(A).find(".checkbox-column").each(
								function(D, C) {
									var E = $(":checkbox", $(C)).prop(
											"checked", z).trigger("change");
									if (E.hasClass("uniform")) {
										$.uniform.update(E)
									}
									$(C).closest("tr")
											.toggleClass("checked", z)
								})
					})
				});
		$(".table-checkable tbody tr td.checkbox-column :checkbox").on(
				"change", function() {
					var x = $(this).prop("checked");
					$(this).closest("tr").toggleClass("checked", x)
				});
		$(".datatable.table-checkable")
				.bind(
						"draw",
						function() {
							var y = $("tbody tr td.checkbox-column :checkbox",
									this).length;
							var A = $(
									"tbody tr td.checkbox-column :checkbox:checked",
									this).length;
							var z = $("thead th.checkbox-column :checkbox",
									this);
							var x = false;
							if (y == A && y != 0) {
								x = true
		...
		$("body")
				.click(
						function(A) {
							...
							if ($.inArray("project-switcher", z) == -1
									&& $.inArray("project-switcher-btn", z) == -1
									&& $(A.target).parents().index(
											$(".project-switcher")) == -1
									&& $(A.target).parents(
											".project-switcher-btn").length == 0) {
								x()
							}
						});
